...
public class DlqConsumer {
    ...
    public DlqConsumer(WikimediaEventRepository eventRepository, FailedEventRepository failedEventRepository) {
    ...
    public void consume(String eventData) {
        ...
        try {
            ...
            failedEventRepository.findByEventData(eventData).ifPresent(failedEvent -> {
            ...
            failedEventRepository.findByEventData(eventData).ifPresentOrElse(
                    failedEvent -> {
                        int newRetryCount = failedEvent.getRetryCount() + 1;
                        if (newRetryCount >= MAX_RETRY_COUNT) {
                            log.error("Event exceeded max retry count ({}), marking as permanently failed", MAX_RETRY_COUNT);
                        }
